...
export function shiftExportRows(list: EnrichedShift[]) {
  return list.map((s) => ({
    ...
    "Supposed pay date": fmtDate(s.officialPayDate, ""),
    "Real pay date": fmtDate(s.expectedPayDate, ""),
    "Real pay days after supposed": s.expectedLateDays ?? "",
    Paid: s.status === "COMPLETED" ? (s.paid ? "Yes" : "No") : "",
    "Actual payment date": fmtDate(s.actualPayDate, ""),
    "Received days after supposed": s.paidDaysAfterOfficial ?? "",
